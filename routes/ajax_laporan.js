  ...
  app.post(['/ajax/laporan_pembayaran_by_metode_bayar.html'],(req, res) => {
    if(req.session.is_login){
      pool.getConnection(function(err, connection) {
        ...
        var sql_data = "SELECT a.metode_bayar,count(a.id) AS jml_transaksi, sum(a.nominal_pembayaran) AS jml_pembayaran, sum(IFNULL(b.biaya_layanan,0)) as biaya_layanan, sum(IFNULL(b.biaya_layanan_duitku,0)) as biaya_layanan_duitku, sum(b.total_pembayaran) as total_pembayaran FROM pembayaran a LEFT JOIN pembayaran_reference b ON a.reference = b.reference LEFT JOIN member c on a.member_id=c.id LEFT JOIN ppp_secret d on c.ppp_secret_id=d.id LEFT JOIN server e on d.server_id=e.id LEFT JOIN user f on e.user_id=f.id where a.bulan = ? and a.tahun=? and (f.id=? or f.id=?) group by a.metode_bayar";
