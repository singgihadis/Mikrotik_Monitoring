  ...
  app.post(['/ajax/ping_data.html'],(req, res) => {
    if(req.session.is_login){
      pool.getConnection(function(err, connection) {
        ...
        var sql_data = "select a.* from ping_data a inner join monitoring b on a.monitoring_id=b.id where a.tgl >= ? and a.tgl <= ? and b.server_id=?";
        var query_data = connection.query(sql_data,[tgl_start,tgl_end,req.session.server_id], function (err, results, fields) {
          if(results.length == 0){
            connection.release();
