    ...
    success:function(resp){
      ...
      if(res.is_error){
        ...
        $.each(data,function(k,v){
          if(k < 10){
            var upload_max_limit_formated = BitToDigitalStorageUnit(v['upload_max_limit']);
            var download_max_limit_formated = BitToDigitalStorageUnit(v['download_max_limit']);
            html += "<tr>";
            html += "<td>" +  no + "</td>";
            html += "<td>" + v['name'] + "</td>";
...
function insert_data(){
  ...
  $.ajax({
    type:'post',
    url:'/ajax/simple_queue_simpan.html',
    data:{},
    success:function(resp){
      var res = JSON.parse(resp);
      var html = "";
