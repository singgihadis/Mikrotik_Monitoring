...
function insert_data(){
  ...
  $.ajax({
    type:'post',
    url:'/ajax/hotspot_user_simpan.html',
    data:{},
    success:function(resp){
      var res = JSON.parse(resp);
      var html = "";
    ...
    success:function(resp){
      ...
      if(res.is_error){
        if(res.must_login){
          ...
          $("#listdata2").html("<tr><td colspan='7'>" + res.msg + "</td></tr>");
        }
      }else{
        var data = res.data;
        var html = "";
        ...
        $.each(data,function(k,v){
          ...
          html += "<td>" + v['user'] + "</td>";
          html += "<td>" + v['address'] + "</td>";
          html += "<td>" + ParseDNSCacheTime(v['uptime']) + "</td>";
          html += "<td>" + ParseDNSCacheTime(v['idleTime']) + "</td>";
          html += "<td>" + comment + "</td>";
          html += "</tr>";
          no++;
        });
        ...
        $('#table_active').DataTable( {
         ...
         $("#total_aktif").html(FormatAngka(data.length));
      }
    },error:function(){
      $("#listdata2").loading("stop");
      $("#listdata2").html("<tr><td colspan='7'>Silahkan periksa koneksi internet anda</td></tr>");
    }
  });
}
